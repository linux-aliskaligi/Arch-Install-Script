...
echo "${mavi}</> ${sari} Kuracağimiz partion'u biçimlendirelim... ${mavi}</> ${yesil}"
cfdisk
lsblk
partion=
...
echo "${sari}</> ${mavi} Pacstap ile gerekli paketleri sisteme yukleyelim... ${sari}</> ${mor}"
pacstrap /mnt base linux linux-firmware base-devel

echo "${turkuaz}</> ${sari} simdi `genfstab` 'i ayarlayalim... ${turkuaz}</> ${mavi}"
...
echo "${sari}</> ${mor} Bu script ile isiniz bitti, simdi `setup.sh` 'i çalistiriniz. ${sari}</> ${yesil}"
arch-chroot /mnt
