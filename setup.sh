...
echo "${mor}</> ${sari}Bir root parolası belirleyiniz... ${mor}</> ${mavi}"
passwd
...
echo "${sari}</> ${mavi}Kullanici oluşturalim... ${sari}</> ${turkuaz}"
useradd -m user
...
echo "${turkuaz}</> ${mor}Ekran Yöneticisi kuralim... ${turkuaz}</> ${sari}"
dm=
echo -e "\n 1)LightDM(lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings) \n 2)Gdm3(gdm3) 3)Sddm(sddm) \n"
echo -e "Ornegin LightDM yükleyeceksek 1 yazarak veya LightDM yazarak degil parantez icindeki kodu yazarak kurunuz."
read -p 'Kuracagınız masaustu ortami veya pencere yoneticisini seçiniz : ' dm
pacman -S $dm
...
echo -e "Ornegin Xfce yükleyeceksek 1 yazarak veya Xfce yazarak degil parantez icindeki kodu yazarak kurunuz."
read -p 'Kuracagınız masaustu ortami veya pencere yoneticisini seçiniz : ' desktop_wm
pacman -S $desktop_wm
...
echo "${gri}</> ${kirmizi}Hadi görüşürüz ben kaçar işim buraya kadardı sonraki scriptlerde görüşmek üzere :D ${gri}</> ${mavi}"
exit
umount -R /mnt
reboot
